...
def build_text(R):
    ...
    T['variant'] = (
        'For the BERT variant we chose DeBERTa, specifically the microsoft/deberta-v3-base checkpoint. Three '
        'properties made it the right choice for this problem rather than simply a newer model. First, its '
        'attention mechanism keeps the representation of a word and the representation of its position '
        'separate, which suits a task where the way text is written may matter as much as which words appear. '
        'Second, it is pretrained with a replaced-token objective that is more efficient than ordinary masked '
        'language modelling at a similar size, so it tends to do better after only a few epochs of '
        'fine-tuning, which is what our budget allowed. Third, its tokeniser treats a space before a '
        'punctuation mark as part of the token, whereas the BERT tokeniser discards that distinction '
        'entirely. That gives the two models genuinely different views of the same text, which turned out to '
        'matter and is discussed in Section 6.\n'

...
REP_COLOR = {'BoW': '#7BA3CC', 'TF-IDF': '#4477AA', 'TF-IDF-s': '#2E5580',
             'char3-5': '#EE6677'}
STAGE_COLOR = {1: '#4477AA', 2: '#4477AA', 3: '#AA3377', 4: '#AA3377', 5: '#228833'}
SHADOW = '#000000'
ARROW_COLOR = '#5B6B7A'
...
def main():
    ...
    def shadow(x, y, w, h, rounding=1.4, dx=0.28, dy=-0.32):
        ax.add_patch(FancyBboxPatch((x + dx, y + dy), w, h,
                                    boxstyle=f'round,pad=0.35,rounding_size={rounding}',
                                    fc=SHADOW, ec='none', alpha=0.06, zorder=1))

    def box(x, y, w, h, title, sub=None, fc='#FAFAFA', ec='#C9CDD2',
            tfs=8.2, sfs=7.0, lw=0.9, card=True):
        if card:
            shadow(x, y, w, h)
        ...
        if sub:
            ax.text(cx, y + h * 0.62, title, ha='center', va='center',
                    fontsize=tfs, fontweight='bold', color='#1A1A1A', zorder=3)
            ax.text(cx, y + h * 0.27, sub, ha='center', va='center',
                    fontsize=sfs, color='#4A4A4A', zorder=3)
        else:
            ax.text(cx, y + h / 2, title, ha='center', va='center',
                    fontsize=tfs, fontweight='bold', color='#1A1A1A', zorder=3)

    def arrow(x1, y1, x2, y2, color=ARROW_COLOR, lw=1.3, rad=0.0):
    ...
    def stage(x, y, n):
        ax.add_patch(FancyBboxPatch((x - 1.55, y - 1.55), 3.1, 3.1,
        ...
        ax.text(x, y, str(n), ha='center', va='center', fontsize=6.8,
                ...
                bbox=dict(boxstyle='circle,pad=0.30', fc=STAGE_COLOR[n], ec='none'))
    ...
    stage(5.6, ys + hs - 0.6, 1)     # acquisition and audit
    stage(49.6, ys + hs - 0.6, 2)    # balancing and partitioning

    ax.plot([79, 79], [ys, 80.5], color='#D5D8DC', lw=1.0, zorder=1)
    ax.plot([19, 79], [80.5, 80.5], color='#D5D8DC', lw=1.0, zorder=1)
    ...
    stage(5.6, yv + hv - 0.6, 3)

    # ---------------------------------------------------------------- band 3
    ym, hm = 40, 22
    box(4, ym, 57, hm, '', None, '#FDFDFD', '#C9CDD2')
    ax.text(32.5, ym + hm - 3.6,
            f'Classical sweep   {n_fam} families   {n_cfg} configurations',
            ha='center', va='center', fontsize=8.4, fontweight='bold', color='#1A1A1A', zorder=3)
    ...
    ax.text(80, ym + 3.0, 'BERT-base (top)    DeBERTa-v3 (bottom)',
            ha='center', va='center', fontsize=6.6, color='#4A4A4A', zorder=3)

    arrow(19, yv, 24, ym + hm)
    arrow(49, yv, 40, ym + hm)
    arrow(80, yv, 80, ym + hm)
    stage(5.6, ym + hm - 0.6, 4)

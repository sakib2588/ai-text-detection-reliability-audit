...
\textbf{Detection on these benchmarks.} \citet{Guo2023close} introduce HC3 with a RoBERTa detector at 99.82 F1, establishing both the corpus and the accuracy regime the field has worked in since, though \citet{Su2023plus} show that regime narrows to roughly 91.7\% under semantic-invariant tasks. \citet{Zhou2024exploiting} reaches competitive DAIGT V2 accuracy with character n-grams and no transformer, and \citet{Kubrusly2025detecting} likewise report strong bag-of-words results on HC3 without isolating what those features encode. \citet{Ansary2026multirepresentation} and \citet{Lamsiyah2025daigt} report near-ceiling DAIGT-family results in-distribution only. \citet{Annepaka2026synergizing} mix linguistic features with a transformer for 99.45\% on HC3, combining the two channels to raise a score. Surveys~\cite{Crothers2023machine} catalogue detector families without asking what a corpus is separable by. The field has since converged on shared evaluations that address the coverage problem rather than the separability one. RAID~\cite{Dugan2024raid} spans eleven generators, eight domains and eleven adversarial attacks, M4~\cite{Wang2024m4} adds multilingual and multi-domain coverage, and SemEval-2024 Task 8~\cite{Wang2024semeval} turned both into a shared task. Those benchmarks make detectors comparable across conditions. They do not ask what any one condition is separable by.

\textbf{Competition solutions on DAIGT V2.} The corpus was assembled for a public competition, and strong public solutions~\cite{Zain2025multi} lean on character n-grams and ensembles of classical models rather than larger language models, usually read as evidence that classical models suffice on this corpus.

\textbf{Documented artefacts in these corpora.} \citet{Tian2023multiscale} identify the HC3 whitespace convention, release a cleaning kit, and go further than they are usually credited for. Their appendix reports that a detector consisting of the single test for one token identifier reaches 82.12 F1 on HC3 at sentence level, above the 81.89 they quote for a fine-tuned RoBERTa. \citet{Baidya2026detecting} document HC3's length confound and control it by length-matching. \citet{Ardeshirifar2025comparing} standardises punctuation before comparing handcrafted features to deep models without measuring what that removes, and \citet{Park2024investigating} show HC3-trained detectors rely on prompt-specific shortcuts.

\textbf{Shortcut learning and dataset artefacts elsewhere.} The pattern is not specific to text detection. \citet{Torralba2011unbiased} showed vision datasets are identifiable from their own images, and in natural language inference a model reading only the hypothesis reaches far above chance~\cite{Gururangan2018annotation,Poliak2018hypothesis}, while \citet{Niven2019probing} traced apparent argument comprehension to lexical cues that vanish under an adversarial split, with \citet{Geirhos2020shortcut} giving the general account. \citet{Feng2019misleading} give the caveat that governs how such a baseline may be read: a partial-input model scoring highly shows a dataset is cheatable, but one scoring poorly does not show the dataset is clean, since the restricted representation may simply fail to express an artefact that is present.

\textbf{Robustness and its controls.} \citet{Antoun2023towards} attack HC3-trained detectors with misspellings and homoglyphs, reporting a fall from 99.88 F1 to 33.57\% accuracy on hand-written adversarial human text, and \citet{Huang2024generated} report comparable degradation before recovering 6.5 to 18.25 accuracy points with a calibrated reconstruction network. Neither reports a label-free control. \citet{Lai2024adaptive} show ensembling preserves in-distribution accuracy while still degrading under shift, \citet{Zhang2024detecting} reframe detection as a two-sample test, and \citet{Borile2025generalize} ablate roughly twenty feed-forward neurons, about 0.05\% of the network, for up to 6.9 points of out-of-distribution gain, evidence that some in-distribution performance rests on localised corpus-specific features.

\textbf{Fairness of the resulting decisions.} \citet{Liang2023gpt} show several deployed detectors misclassify more than half of non-native-authored TOEFL essays as machine-generated while remaining near-perfect on essays by native writers.

\textbf{Cross-corpus evaluation.} \citet{Alikhanov2026generated} merge HC3 and DAIGT V2 into one 124,195-sample corpus under a topic-held-out split, reporting 82.87\% accuracy for logistic regression, 88.86\% for a BiLSTM at 0.94 ROC-AUC and 88.11\% for DistilBERT at 0.96 ROC-AUC, against the near-ceiling numbers usual in-distribution. \citet{Lu2023large} show detectors can be evaded by prompting alone.

Table~\ref{tab:relatedwork} summarises the prior work above, reporting every result each study states rather than a single selected metric.

\begin{table*}[!t]
\centering
\caption{Prior work on these benchmarks, with the full set of results each study reports and its limitations.}
\label{tab:relatedwork}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{@{}clp{2.6cm}p{2.1cm}Xp{2.5cm}@{}}
\toprule
Ref. & Year & Method & Corpus & Reported results, as stated by the authors & Limitation \\
\midrule
\cite{Guo2023close} & 2023 & GLTR and fine-tuned RoBERTa, single and QA variants & HC3 & 99.82 F1 English full text, RoBERTa-QA 97.48 F1 on filtered full text and 5.63 points above the answer-only model, 94.22 best Chinese average, 81.89 F1 at sentence level when trained on full text against 98.43 when trained on sentences, 100.00 F1 on reddit\_eli5, 26.78 worst sentence-level class F1 on open\_qa, 0.90 human expert accuracy & No decomposition of surface and content signal \\

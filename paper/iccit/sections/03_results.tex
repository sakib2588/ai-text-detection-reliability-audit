...
\begin{table}[!ht]
\centering
\caption{HC3 by source domain, each rebalanced to parity within train and test, on the same group-aware split restricted to the domain. The final columns give the share of documents carrying at least one space before punctuation. Domains below 200 test rows are marked, because a paired test there fails to separate for want of power rather than for want of an effect.}
\label{tab:subgroup}
\footnotesize
\setlength{\tabcolsep}{3.5pt}

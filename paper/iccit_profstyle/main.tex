...
\caption{The complete study pipeline. Cleaned word features and raw character n-grams feed the classical sweep, raw subwords feed the transformers, and the analysis layer sits on top of the fitted models rather than replacing them. Counts on the diagram are read from the committed result tables.}
\label{fig:pipeline}
\end{figure*}
...
Figure~\ref{fig:wordclouds} shows the most frequent vocabulary per class after cleaning. The DAIGT V2 panels are dominated by the shared essay topics, so human and machine essays draw on much the same nouns, and the visible difference is one of proportion rather than of subject. The HC3 panels differ more visibly, with contracted forms concentrated on the human side and hedging and discourse words on the machine side. Frequency alone does not separate the classes, which is why the analysis in Section~\ref{sec:terms} moves to log-odds and to fitted coefficients rather than to raw counts.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{wordclouds.pdf}
\caption{Most frequent vocabulary by class and corpus after cleaning. Size is frequency within the class. The two DAIGT V2 panels share their topical vocabulary almost entirely.}
...
Table~\ref{tab:families} reports the best configuration of every classical family on both corpora, and Figure~\ref{fig:ranking} draws the same ranking. Three things in it matter for the rest of the paper.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{model_ranking.pdf}
\caption{Best configuration per model family, ranked by weighted F1 within each corpus, with the winning representation printed beside each bar. The ordering is not shared between the two corpora.}
\label{fig:ranking}
\end{figure*}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{DAIGT_mean_absolute_shap_value.pdf}
\caption{DAIGT V2 mean absolute SHAP value per term over the LightGBM model. The leading terms are ordinary essay vocabulary and no single term dominates.}
\label{fig:shapbardaigt}
\end{figure}

\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{HC3_mean_absolute_shap_value.pdf}
\caption{HC3 mean absolute SHAP value per term over the LightGBM model. The distribution is far more concentrated, with \texttt{important} carrying roughly 1.6 times the weight of the second term.}
\label{fig:shapbarhc3}
\end{figure}
...
Shapley attribution~\cite{Lundberg2017unified} over the fitted LightGBM models gives the per-document view. Figures~\ref{fig:shapbardaigt} and~\ref{fig:shapbarhc3} rank terms by mean absolute contribution, and the shapes of the two rankings differ as much as their contents. DAIGT V2 declines gradually from \texttt{would} at 0.86 through a long tail of essay vocabulary, so no term dominates. HC3 is led by \texttt{important} at 0.77 with \texttt{help} and \texttt{url} close behind, and the decline after those is much steeper.

Figures~\ref{fig:shapswarmdaigt} and~\ref{fig:shapswarmhc3} show the same attributions per document. The HC3 panel is the informative one, since the \texttt{url} row carries a long negative tail reaching below $-4$, meaning that a document containing the placeholder token is pushed hard towards the human label. No DAIGT V2 feature produces a tail of that shape. A single collection token acting as near-decisive evidence for one class is precisely the phenomenon the decomposition below quantifies without needing to name the feature in advance.
...
On DAIGT V2 the leading contributors are document length at 2.77, mean word length at 1.88 and casing statistics, none of which exceeds 2.8 and none of which is a collection convention in the same sense. The surface arm on that corpus is reading a diffuse stylistic signal rather than a single artefact, which is consistent with its much weaker showing against the content arm and with the diffuse term-level attribution in Figure~\ref{fig:shapbardaigt}.
...
Three candidate mechanisms were tested and none survives. Majority-prior collapse is excluded by the training balance shown in Figure~\ref{fig:balance}, 0.4981 against 0.5019 on DAIGT V2 and 0.4999 against 0.5001 on HC3. Cue injection is excluded because the perturbation does not create the whitespace artefact, moving the machine-side count only from 0.000 to 0.005. And a learned association from disfluency to human authorship is unsupported, since subword fertility runs the wrong way, 1.1192 for HC3 human text against 1.1980 for machine.

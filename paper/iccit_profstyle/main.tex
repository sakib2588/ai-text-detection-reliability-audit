...
% Explicit, viewer-independent link colours (bare \usepackage{hyperref} falls
% back to unstyled border-box annotations that most modern PDF viewers
% silently suppress, so citations/cross-references/DOI links can look
% entirely unstyled with no visual distinction at all). Colours reuse this
% paper's own colourblind-safe palette (fig_pipeline.pdf, fig_dashboard.pdf,
% etc.): green for \cite, a muted red for internal \ref (Section/Table/
% Figure/equation), blue for \url (DOI links in the bibliography).
\definecolor{citegreen}{HTML}{1A7A1A}
\definecolor{refred}{HTML}{A0282E}
